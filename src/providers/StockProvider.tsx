...
export type DenominationStockUpdateAction = ReducerActionMap<
  DenominationStockUpdatePayload
>[keyof ReducerActionMap<DenominationStockUpdatePayload>];

function stockIsUpdatable(
...
const StockProvider: React.FC = ({ children }) => {
  const [state, updatedStockState] = useReducer(
    updateStockReducer,
    initalState
  );
  const stockIsDeductable = (
    ty: DenominationTypes,
    chBy: number = 1
  ): boolean => {
  ...
  return <Provider value={newState}>{children}</Provider>;
};
